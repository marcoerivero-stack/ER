...
async function guardarEstado(persona, estado) {
  try {
    await fetch(`${API_URL}?modo=set&persona=${encodeURIComponent(persona)}&estado=${encodeURIComponent(JSON.stringify(estado))}`);
  } catch(err) {
...
function renderTabla(tabla, alimentos, estado, persona) {
  ...
  Object.entries(alimentos).forEach(([alimento, columnas]) => {
    ...
    columnas.forEach(porciones => {
      ...
      porciones.forEach(p => {
        // ===== NUEVO: wrapper con numero arriba =====
        const wrap = document.createElement("div");
        wrap.className = "chk-wrap";

        const label = document.createElement("span");
        label.className = "chk-label";
        label.textContent = String(p); // "1" o "0.5"
